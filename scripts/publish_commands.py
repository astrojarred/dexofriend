...
def overwrite_all_commands(url, commands):
    ...
    if not r.ok:
        # pinging the endpoint too frequently causes it to fail; wait and retry
        sleep(20)
        print(f"Put to {url} failed; retrying once")
        r = requests.put(url, headers=HEADERS, json=commands)

        if not r.ok:
            print(f"Response from {url}: {r.text}")
            raise RuntimeError("Failed to update commands!")
...
def run():
    ...
    #         commands[i]["name"] = f"{command_name}_{STAGE}"
